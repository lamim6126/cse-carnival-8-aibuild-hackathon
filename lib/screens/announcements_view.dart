...
class _AnnouncementsViewState extends State<AnnouncementsView> {
  ...
  Widget build(BuildContext context) {
    ...
    final filtered = db.announcements.where((a) {
            ...
            children: [
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text("Campus Announcements & Notices", style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
                    Text("${db.announcements.length} official notices posted", style: TextStyle(color: Colors.grey.shade600)),
                  ],
                ),
              ),
              ElevatedButton.icon(
                style: ElevatedButton.styleFrom(backgroundColor: Colors.orange.shade800, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12)),
            ...
            children: [
              ...
              Expanded(
                ...
                child: DropdownButtonFormField<String>(
                  ...
                  items: priorities.map((p) => DropdownMenuItem(value: p, child: Text(p == 'All' ? 'All Priorities' : p.toUpperCase()))).toList(),
            ...
            child: filtered.isEmpty
                ...
                : ListView.separated(
                    ...
                    itemBuilder: (ctx, i) {
                      ...
                      Color badgeColor = isHigh ? Colors.red : (isMed ? Colors.orange : Colors.blue);
                                ...
                                children: [
                                  ...
                                  IconButton(
                                    ...
                                    onPressed: () async {
                                      ...
                                      if (confirm == true) db.deleteAnnouncement(item.id);
                                    },
                                  ),
                                ],
                              ),
                              const SizedBox(height: 8),
                              Text(item.body, style: TextStyle(color: Colors.grey.shade800, fontSize: 14, height: 1.4)),
                              const SizedBox(height: 12),
                              Row(
                                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                children: [
                                  Text("Posted by: ${item.postedBy}", style: TextStyle(fontSize: 12, color: Colors.grey.shade600, fontWeight: FontWeight.w500)),
                                  Text("Posted: ${item.date}  |  Expires: ${item.expires}", style: TextStyle(fontSize: 12, color: Colors.grey.shade600)),
                                ],
                              ),
                            ],
                          ),
