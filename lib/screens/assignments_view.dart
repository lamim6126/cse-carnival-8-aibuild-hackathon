...
class _AssignmentsViewState extends State<AssignmentsView> {
  ...
  Widget build(BuildContext context) {
    ...
    final filtered = db.assignments.where((a) {
            ...
            children: [
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text("Course Assignments & Deadlines", style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
                    Text("${db.assignments.length} total assignments tracked", style: TextStyle(color: Colors.grey.shade600)),
                  ],
                ),
              ),
              ElevatedButton.icon(
                style: ElevatedButton.styleFrom(backgroundColor: Colors.blueGrey.shade800, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12)),
                            ...
                            children: [
                              Row(
                                children: [
                                  Container(
                                    ...
                                    child: Text(item.course, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Colors.blueGrey.shade900)),
                                  ),
                                  ...
                                  IconButton(
                                    ...
                                    onPressed: () async {
                                      ...
                                      if (confirm == true) db.deleteAssignment(item.id);
                                    },
                                  ),
                                ],
                              ),
                              const SizedBox(height: 8),
                              Text(item.description, style: TextStyle(color: Colors.grey.shade800, fontSize: 14)),
                              ...
                              Row(
                                ...
                                children: [
                                  Row(
                                    children: [
                                      const Icon(Icons.calendar_month, size: 16, color: Colors.redAccent),
                                      const SizedBox(width: 6),
                                      Text("Due Date: ${item.deadline}", style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 13, color: Colors.redAccent)),
                                    ],
                                  ),
                                  Text("Assigned: ${item.assignedDate}", style: TextStyle(fontSize: 12, color: Colors.grey.shade600)),
                                ],
                              ),
                            ],
                          ),
